...
const Sidebar = () => {
  ...
  return (
    <>
      ...
      <div className="fixed md:static top-0 left-0 h-full w-16 md:w-16 bg-accent flex flex-col items-center py-4 space-y-4 z-40">
        {navItems.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
      ...
      <div className={`${isOpen ? 'translate-x-0' : 'translate-x-full'} transition-transform duration-300 ease-in-out fixed top-0 right-0 h-full w-48 bg-accent flex flex-col items-start py-4 px-2 space-y-4 z-30`}>
        {navItems.map((item) => (
          <NavLink
            key={item.to}
            to={item.to}
